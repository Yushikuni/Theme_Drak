...
if ($ADMIN->fulltree) {                                                                                                             
    ...
    $settings = new theme_boost_admin_settingspage_tabs('themesettingphoto', get_string('configtitle', 'theme_drak'));             
 
    // Each page is a tab - the first is the "General" tab.                                                                         
    $page = new admin_settingpage('theme_drak_general', get_string('generalsettings', 'theme_drak'));                             
 
    // Replicate the preset setting from boost.                                                                                     
    $name = 'theme_drak/preset';                                                                                                   
    $title = get_string('preset', 'theme_drak');                                                                                   
    $description = get_string('preset_desc', 'theme_drak');                                                                        
    ...
    $setting = new admin_setting_configtextarea('theme_drak/scsspre',                                                              
        get_string('rawscsspre', 'theme_drak'), get_string('rawscsspre_desc', 'theme_drak'), '', PARAM_RAW);                      
    $setting->set_updatedcallback('theme_reset_all_caches');                                                                        
    $page->add($setting);                                                                                                           
 
    // Raw SCSS to include after the content.                                                                                       
    $setting = new admin_setting_configtextarea('theme_drak/scss', get_string('rawscss', 'theme_drak'),                           
